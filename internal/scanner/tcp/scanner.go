...
func Ports(preset string, custom []int) []int {
	m := map[int]bool{}
	for _, p := range presets[preset] { m[p] = true }
	for _, p := range custom { if p > 0 && p <= 65535 { m[p] = true } }
	out := make([]int, 0, len(m))
	for p := range m { out = append(out, p) }
	sort.Ints(out)
	return out
}

func Scan(ctx context.Context, addresses []netip.Addr, ports []int, timeout time.Duration, concurrency int) map[string][]int {
	type job struct { addr netip.Addr; port int }
	jobs := make(chan job)
...
send:
	...
	close(jobs)
	wg.Wait()
	for k := range result { sort.Ints(result[k]) }
	return result
}
